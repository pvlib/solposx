...
def sg2(elevation, pressure=101325., temperature=12.):
    ...
    refraction = (
        (pressure * 283 * 2.96706 * 10**-4) / (1010 * (273 + temperature)
                                               * np.tan(
                       elevation_rad + 0.0031376 * (elevation_rad + 0.089186)**-1)))

    low_elevation_mask = elevation_rad <= -0.01
    refraction[low_elevation_mask] = (
        (-pressure * 283 * 1.005516*10**-4) /
        (1010 * (273 + temperature) * np.tan(elevation_rad)))[low_elevation_mask]
